...
def top_vars(variants, ordered, pct):
    ...
    with open(ordered, 'r') as f:
        ordered_vars = [(v.split(',')[0],int(v.split(',')[1])) for v in f.readline().split('\t')[:-1]]
    print(ordered_vars[:10])
...
def go(args):
    ...
    genome = iohelp.read_genome1(args.reference, None)
    print('genome check')
    variants = iohelp.parse_1ksnp(args.vars)
    print('variants check')
    if args.sorted:
