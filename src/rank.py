...
def go(args):
    ...
    x2 = 'Memory usage after genome is created: %s (kb)' % resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
    vars = iohelp.parse_1ksnp(args.vars)
    x3 = 'Memory usage after variants is created: %s (kb)' % resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
    ranker = VarRanker(genome, vars, r, args.phasing, max_v)
    if args.pseudocontigs:
        ranker.seen_pcs(o)
    else:
        ranker.rank(args.method, o)
    x4 = 'Memory usage after ranking: %s (kb)' % resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
    x5 = "Running time: %s seconds" % (time.time() - start_time)    
    oram = 'ram-'+args.method+'.txt'
